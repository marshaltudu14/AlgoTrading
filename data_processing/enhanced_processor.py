"""
Enhanced data processor with advanced feature engineering for algorithmic trading.
...
def process_df(df, rr_ratio=2, multi_timeframe=True, normalize=True):
    ...
    if 'datetime' in df.columns:
        ...
        try:
            ...
            print("Could not calculate days to expiry")
...
def process_all(input_dir='historical_data', output_dir='processed_data', rr_ratio=2, normalize=True, force=False):
    """
    ...
    if not force and not should_process_data(sys.modules[__name__], output_dir):
        logger.info(f"Skipping enhanced processing: feature definitions unchanged")
        return False
    ...
    logger.info(f"Found {len(csv_files)} CSV files to process with enhanced features")
    ...
    for fname in csv_files:
        ...
        logger.info(f"Saved processed file: {out_path} with {len(feature_cols)} enhanced features")
