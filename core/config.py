"""
Configuration settings for the AlgoTrading system.
Centralized configuration to manage all settings in one place.
"""
import os
import torch
...
HIST_DIR = os.getenv("HIST_DIR", str(BASE_DIR / "historical_data"))
PROCESSED_DIR = os.getenv("PROCESSED_DIR", str(BASE_DIR / "processed_data"))
...
# Lot quantities per instrument (standard lot sizes for Indian indices)
QUANTITIES = {
    'Bankex': 15,
    'Finnifty': 40,
    'Bank Nifty': 30,
...
# Margin requirements for each instrument (percentage of contract value)
MARGIN_REQUIREMENTS = {
...
MODEL_PATH = os.getenv("MODEL_PATH", str(BASE_DIR / "models" / "trading_transformer.pt"))
MOE_MODEL_PATH = os.getenv("MOE_MODEL_PATH", str(BASE_DIR / "models" / "moe_model.pt"))
...
PRE_MARKET_MINUTES = 15  # Minutes before market open to prepare
POST_MARKET_MINUTES = 15  # Minutes after market close to finalize

# Option trading settings
MAX_DAYS_TO_EXPIRY = 7  # Maximum days to expiry for options to trade
MIN_DAYS_TO_EXPIRY = 1  # Minimum days to expiry for options to trade
PREFER_ATM = True  # Prefer At-The-Money options over ITM
